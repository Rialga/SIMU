...
public class ListMahasiswaActivity extends AppCompatActivity {
    ...
    TextView matkul,ruangan,tanggal,mulai,akhir;
    ...
    private void fill(JSONObject detailAwas) {
        ...
        tanggal = findViewById(R.id.mtgl);
        ruangan = findViewById(R.id.mruang);
        mulai = findViewById(R.id.mawal);
        akhir = findViewById(R.id.makhir);
        baseAPIService = UtilsAPI.getAPIService();

        try {
            matkul.setText(detailAwas.getString("nama_mk"));
            ruangan.setText(detailAwas.getString("ruangan"));
            tanggal.setText(detailAwas.getString("tanggal_ujian"));
            mulai.setText(detailAwas.getString("mulai"));
            akhir.setText(detailAwas.getString("selesai"));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private void ListMahasiswa(String token, int kelas_id, int ruangan_id) {
        baseAPIService.mahasiswaRequest(token,String.valueOf(kelas_id),String.valueOf(ruangan_id)).enqueue(new Callback<ResponseBody>(){
